...
class RestExceptionHandler : ResponseEntityExceptionHandler() {
    ...
    override fun handleHttpMessageNotReadable(ex: HttpMessageNotReadableException, headers: HttpHeaders, status: HttpStatus, request: WebRequest): ResponseEntity<Any> {
        return ResponseEntity.badRequest().body(ApiError("The request-body could not be parsed (bad semantics)"))
    }
    ...
    override fun handleHttpRequestMethodNotSupported(ex: HttpRequestMethodNotSupportedException, headers: HttpHeaders, status: HttpStatus, request: WebRequest): ResponseEntity<Any> {
        return ResponseEntity(ApiError("This endpoint does not support the ${ex.method} method"), HttpStatus.METHOD_NOT_ALLOWED)
    }
    ...
    override fun handleBindException(ex: BindException, headers: HttpHeaders, status: HttpStatus, request: WebRequest): ResponseEntity<Any> {
        return ResponseEntity.badRequest().body(ApiError("The request-parameters did not pass validation", convertBindException(ex)))
    }

    // Fields within the body were not valid (restriction validation)
    override fun handleMethodArgumentNotValid(ex: MethodArgumentNotValidException, headers: HttpHeaders, status: HttpStatus, request: WebRequest): ResponseEntity<Any> {
        return ResponseEntity.badRequest().body(ApiError("The request-body did not pass validation", convertBindException(ex.bindingResult)))
    }
